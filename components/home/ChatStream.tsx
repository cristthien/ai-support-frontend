...
import chatApiRequest, { StreamCallbacks } from "@/apiRequests/chat";
import MarkdownMessage from "@/components/chat/MarkdownMessage";
import { SourceType, MessageSourceType, SearchModeType, PipelineModeType, StreamMetadataEvent } from "@/schemaValidation/chat.schema";
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
interface Message {
    ...
    metadata?: StreamMetadataEvent["data"];
    isStreaming?: boolean;
}
...
const ChatStream: React.FC<ChatStreamProps> = ({ className, initialChatId = null }) => {
    ...
    const handleSend = async (question?: string) => {
        ...
        try {
            console.log("Sending message with streaming:", chatId);

            const callbacks: StreamCallbacks = {
                onChatInfo: (newChatId) => {
                    console.log("Stream: chat_info received, chatId:", newChatId);
                    setChatId(newChatId);
                },
                onMetadata: (metadata) => {
                ...
                onSources: (sources) => {
                    console.log("Stream: sources received:", sources.length);
                ...
                onError: (error) => {
                    ...
                    setMessages((prev) =>
                        prev.map((msg) =>
                            msg.id === streamingMessageIdRef.current
                                ? { ...msg, content: `⚠️ **Error**: ${error}`, isStreaming: false }
                                : msg
                        )
                    );
                    setIsLoading(false);
            ...
            await chatApiRequest.sendMessageStream(
                {
                    chat_id: chatId,
                    role: "user",
                    content: messageText,
                        ...
                        <div className="w-full max-w-[700px] space-y-8">
                            ...
                            <div className="text-center space-y-3">
                                ...
                                <p className="text-gray-500 text-lg">
                                    Trợ lý AI với real-time streaming - Hỏi bất cứ điều gì!
                                </p>
                            </div>
